...
Local verification on 2026-09-03: seller feature suite passed; one public page was parsed successfully (not a guarantee for all Yahoo listing types). Isolated SQLite browser checks covered login, empty/list states, registration from inventory, fee edits, refresh and two observations, duplicate errors, no console errors on the successful workflow, and 390px mobile width. On 2026-09-09, the development PostgreSQL database applied migrations through `0019_sale_record_phase4`; the seller outcome integration tests passed 3 cases and `verify --feature seller-outcomes` passed, including the PostgreSQL nullable-join lock regression. Production DB migration, production PostgreSQL concurrency, and GitHub CI remain separate evidence.

When local `DEBUG` is false, `run_systema.bat` runs `collectstatic --noinput` before starting `runserver --insecure`. This prevents newly added seller JavaScript from returning a 404 HTML response with a MIME mismatch. The seller script must remain discoverable through Django staticfiles, and the launcher must be restarted after static asset changes.
...
Phase 4A adds a server-derived `actionStatus` to each listing. It distinguishes missing costs or prices, stale public observations, loss risk, target shortfall, ending soon without bids, bid stagnation, manual sold/cancelled/relist states, and an OK state. The UI renders one next action and reason per card; it does not infer a sale from public closure. Rules are covered by `tests/unit/test_seller_status.py`.
