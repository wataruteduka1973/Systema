...
**Status:** 方針変更。ユーザーページへの機能追加を停止し、出品・在庫管理画面に集約する。購入判断の追加支援は行わず、既存のPhase 3Cは維持する。
...
- Phase 7Aの販売結果入力と確定利益へ接続できるよう、出品時点の想定値を保持する。
...
- 基礎となる販売結果入力・確定利益・在庫/出品の次の作業表示をPhase 7A、高度な推奨や学習をPhase 7Bとする。7Bは販売実績の蓄積後に着手し、7Aの提供を待たせない。
