...
## Phase 3C purchase budget contract

- `GET /api/v1/cost-settings`: own nullable cost defaults and at most 50 recent successful closed-search evidence choices. `PUT` replaces the cost template. Keys: `feeRate`, `purchaseShipping`, `shippingCost`, `packagingCost`, `otherCost`, `targetProfit`. Empty/null is unknown; explicit 0 is known. Sale price is never a global default.
- `GET /api/v1/watch-items/{id}/purchase-budget`: latest saved decision, defaults, current observed price and inventory lock status. `POST`: `assumptions`, optional `evidenceRunId`, `evidenceNote`, `useDefaults`. Product values override defaults; omitted product fields retain saved values, explicit null overrides to unknown. `useDefaults` reapplies only costs, preserving the sale price.
- All endpoints require login and owner scoping; writes require CSRF. 401 unauthenticated, 404 other owner/missing evidence, 400 invalid/unknown fields or acquired decision edits, 405 unsupported method. Money: integer yen 0 through 1 trillion. Fee: 0 through 1, at most five fractional digits, half-up yen rounding.
- Sale evidence is explicit manual price plus required explanatory note, or the median of an owned successful closed SearchRun (1–5000 retained rows). Even-sized medians round half-up to yen. No live search occurs. Snapshot stores references, count, conditions and observation time; actual sale-date range is unknown and never inferred from SearchDay. This is the whole retained sample, not automatic model/condition matching; exclusions remain Phase 4.
- Output states: `insufficient` (null amounts and missing keys), `no_budget` (negative algebraic limit; displayed limit null), `within_budget`, `over_budget`. Limit excludes acquisition shipping; shipping is a separate expense. Current bid is an observation, not a confirmed purchase price.
- Identical saves at the same observation are idempotent. Changed assumptions/evidence/observations append a decision. Conversion copies the latest decision atomically under the watch lock, and existing conversion retries retain the first cost and decision. Global edits never rewrite decisions. Inventory simulation starts from the copied assumptions and persisted actual acquisition cost.
- Inventory and seller responses add `purchaseDecision`. Seller inputs add `purchaseShippingCost` without changing existing shipping fields. New listings copy cost estimates, price, target and immutable evidence from inventory before explicit listing overrides. `missingCostFields` preserves unknown inherited costs; profit stays null until those fields are explicitly filled. New seller observations can have null `estimatedFee`/`estimatedProfit` when costs are unknown; legacy known-cost observations retain numeric outputs. Snapshot calculation inputs record unknown fields and acquisition shipping (also included in total shipping).
