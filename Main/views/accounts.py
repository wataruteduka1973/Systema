...
class SystemaLoginView(LoginView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["admin_setup_available"] = not get_user_model().objects.filter(
            is_superuser=True
        ).exists()
        return context
...
def profile(request):
    ...
    selected_items = (
        selected_run.items.order_by("-id")[:50] if selected_run is not None else []
    )
    watch_items = WatchItem.objects.filter(user=request.user).order_by(
        "-buy_score", "-updated_at"
    )[:20]
    ...
    context = {
        ...
        "search_count": search_runs.count(),
        "scraped_item_count": SearchRun.objects.filter(user=request.user).aggregate(
            total=Count("items")
        )["total"],
