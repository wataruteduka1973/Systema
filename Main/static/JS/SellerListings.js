(() => {
    ...
    async function load() {
        ...
        if (!list.hasChildNodes()) list.append(el('p', '出品一覧を読み込み中…', 'text-muted'));
        try {
            const status = document.getElementById('sellerStatus').value;
            const result = await api(`${base}?page=${page}&status=${encodeURIComponent(status)}`);
            if (current !== loadId) return;
            if (!result.data.length && page > 1) { page--; return await load(); }
    ...
    function card(item) {
        ...
        actions.append(button('公開情報を更新', async () => {
