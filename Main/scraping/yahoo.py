    ...
    def extract_listing_items(cls, html):
        ...
        if product_cards:
            for card in product_cards:
                ...
                auction_id = card.get('data-auction-id') or card.get('data-item-id') or card.get('id') or re.search(r'z\d+', href or '')
                if isinstance(auction_id, re.Match):
                    auction_id = auction_id.group(0)
                auction_id = str(auction_id) if auction_id else None

                if re.search(r'^(yahoo!\s*japan|help|ヘルプ|ログイン|マイページ|検索|トップ|カテゴリ|お問い合わせ|規約|プライバシー)$', title_text, re.I):
                    continue

                price_tag = card.select_one('.Product__priceValue:not(.Product__priceValue--start), .Product__priceValue, .price, [class*="Product__priceValue"]')
                price_text = price_tag.get_text(' ', strip=True) if price_tag else ''

                bid_tag = card.select_one('.Product__bid, dd[class*="Product__bid"], [class*="Product__bid"]')
                ...
                key = str(auction_id or (re.search(r'z\d+', href or '') and re.search(r'z\d+', href or '').group(0)) or title_text)
                if key in seen_ids:
                    continue
                seen_ids.add(key)
                ...
                data = {
                    ...
                    'url': cls.build_item_url(href, auction_id, {'isFleamarketItem': 'paypayfleamarket.yahoo.co.jp' in href or 'item/' in href}),
                }
                if not data['url'] or data['url'] == '#':
                    data['url'] = cls.build_item_url(href or '', auction_id, {'isFleamarketItem': 'paypayfleamarket.yahoo.co.jp' in href or 'item/' in href})
        ...
        for card in soup.select('article, li, div, tr'):
            ...
            title_tag = card.select_one('a[href*="/jp/auction/"], a[href*="/item/"], a[href*="yahoo.co.jp/jp/auction/"], a[href*="paypayfleamarket.yahoo.co.jp/item/"], h3, h4, span.title, .title, .item-name')
        ...
        if raw_url:
            ...
            if value.startswith('/item/') or value.startswith('item/') or value.startswith('z'):
                if value.startswith('z'):
                    return f'https://paypayfleamarket.yahoo.co.jp/item/{value}' if auction_id is None or value.startswith(str(auction_id)) else f'https://paypayfleamarket.yahoo.co.jp/item/{auction_id}'
                return f'https://paypayfleamarket.yahoo.co.jp/{value}' if value.startswith('item/') else f'https://paypayfleamarket.yahoo.co.jp/item/{value.lstrip('/')}'
